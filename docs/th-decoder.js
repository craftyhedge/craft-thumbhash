...
; (function () {
    ...
    function countAcCoefficients(nx, ny) {
        var count = 0;
    ...
    function thumbHashToRGBA(hash) {
        ...
        if (lx_raw <= 0 || ly_raw <= 0) {
            throw new Error('Invalid ThumbHash');
        }

        var lx = max(3, lx_raw);
        var ly = max(3, ly_raw);

        var acCount = countAcCoefficients(lx, ly)
            + countAcCoefficients(3, 3)
            + countAcCoefficients(3, 3)
            + (hasAlpha ? countAcCoefficients(5, 5) : 0);
        var acStart = hasAlpha ? 6 : 5;
        var requiredLength = acStart + Math.ceil(acCount / 2);
